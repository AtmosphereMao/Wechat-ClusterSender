...
$.fn.uploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span><span class="uploadbtn">上传</span><span class="delfilebtn">关闭</span></div>';
	var defaults = {
        fileTypeExts:'*.jpg;*.jpeg;*.png;*.zip;*.rar;*.docx;*.xlsx;*.mp4;*.flv;*.avi;*.txt;',//允许上传的文件类型，格式'*.jpg;*.doc'
		uploader:'',//文件提交的地址
		auto:false,//是否开启自动上传
		...
		fileSizeLimit:50240,//允许上传的文件大小，单位KB
				  ...
				  xhr.onreadystatechange = function(e) {
					  if (xhr.readyState == 4) {
						  if (xhr.status == 200) {
							  ...
							  option.onUploadError&&option.onUploadError(file, xhr);
						  }
